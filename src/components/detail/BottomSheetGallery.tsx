          ...
          >
            ...
            <div className="px-6 py-3 border-b border-gray-100 overflow-x-auto">
              <div className="flex items-center gap-3 min-w-max">
                {sections.map((s, i) => (
                  <button key={s.title} onClick={() => setActive(i)} className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap ${i === active ? 'text-white' : 'text-gray-600'}`} style={i === active ? { backgroundColor: brandBlue } : { backgroundColor: '#F5F0E6' }}>
                    {s.title}
                  </button>
                ))}
              </div>
